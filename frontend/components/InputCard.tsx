...
export default function InputCard({
  ...
  // Arrow-key keyboard navigation for tabs
  ...
  return (
    <div className="bg-card border border-hairline rounded-2xl overflow-hidden shadow-sm">
        ...
        {TABS.map((tab, idx) => {
          ...
          return (
            <button
              ...
              className={`flex items-center justify-center gap-1.5 px-2 py-3 text-xs font-mono uppercase tracking-wider transition-colors focus-visible:z-10 border-r border-hairline last:border-r-0 max-[480px]:even:border-r-0 max-[480px]:nth-[n+3]:border-t ${
                isActive
                  ? "bg-ink text-paper"
      ...
      {/* ── Tab content ──────────────────────────────────────────────────── */}
      <div className="p-5 pb-4 min-h-[120px]">
        <AnimatePresence mode="wait" initial={false}>
          ...
          >
            {activeTab === "text" && (
              <TextTab value={text} onChange={setText} />
            )}
            {activeTab === "article" && (
              <ArticleTab value={articleUrl} onChange={setArticleUrl} />
            )}
            {activeTab === "screenshot" && (
              <ScreenshotTab file={screenshotFile} onFile={setScreenshotFile} />
            )}
            {activeTab === "youtube" && (
              <YoutubeTab value={youtubeUrl} onChange={setYoutubeUrl} />
            )}
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div className="px-5 pb-4 flex justify-end">
        <motion.button
          ...
          className={`px-5 py-2.5 rounded-full text-sm font-medium transition-colors ${
            canSubmit
              ? "bg-ink text-paper cursor-pointer hover:bg-ink/90"
